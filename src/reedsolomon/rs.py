...
class RS:
	...
	def __init__(self, k : int, n : int):
		...
		if k < 1:
			raise ValueError(
				f'k must be a positive integer; k={k}'
			)

		if n < 1:
			raise ValueError(
				f'n must be a positive integer; n={n}'
			)

		if k > n:
			raise ValueError(
				f'k must be less than n; k={k}, n={n}'
			)
		...
		self.eval_points = tuple(
			gf.primitive_element ** i for i in range(n)
		)
	...
	def encode(self, block : Block) -> Block:
		...
		out = Block(
			out_arr, self.n, block.underlying_len(), encoded=True
		)
		return out
