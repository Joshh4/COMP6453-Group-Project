...
class RootsOfUnity:
    ...
    def fft(self, values: list) -> list:
        ...
        while length <= m:
            half = length // 2
            w_step = pow(self.omega, m // length, p)  # twiddle factor for this pass
...
class SRS:
    def __init__(self, g1_powers: list, g2_powers: list, g2_tauD: object, max_degree: int):
    ...
    def __eq__(self, other):
        ...
        return (self.g1_powers == other.g1_powers
...
class KZGSetup:
    ...
    def generate(max_degree: int, D: int = D_CELL_SIZE) -> SRS:
        ...
        return SRS(g1_powers=g1_powers, g2_powers=g2_powers,
                   g2_tauD=g2_tauD, max_degree=max_degree)
...
def _commit_poly(srs: SRS, poly: Polynomial):
    ...
    for i, coeff in enumerate(poly.coeffs()):
        ...
        result = add(result, multiply(srs.g1_powers[i], int(coeff) % FIELD_PRIME))
    return result
...
class KZGProver:
    ...
    def multi_open(self, poly: Polynomial, cell_index: int):
        ...
        cell_values = [
            poly(self.domain.element(cell_index * D + i))
            for i in range(D)
        ]
...
class KZGVerifier:
    ...
    def multi_verify(self, commitment, cell_index: int, cell_values: list, proof) -> bool:
    ...
    def batch_verify(self, commitments: list, cell_indices: list,
                     row_indices: list, cells: list, proofs: list) -> bool:
        L = len(proofs)
        ...
        for ik, jk, cell, pi in zip(row_indices, cell_indices, cells, proofs):
            ...
            I_tau = _commit_poly(self.srs, self.domain.lagrange_interpolate_coset(D, jk, cell))
            term  = add(add(commitments[ik], neg(I_tau)), multiply(pi, coset_root))
...
class CCrow:
    ...
    def __init__(self, srs: SRS, domain: RootsOfUnity,
                 D: int = D_CELL_SIZE, k: int = K_CELLS_BLOB, n: int = N_CELLS_EXT):
        self.srs      = srs
        self.domain   = domain
        self.D        = D
        self.k        = k
    ...
    def commit(self, blob_data: list):
        ...
        assert len(blob_data) == self.D * self.k, (
    ...
    def verify(self, commitment, cell_index: int, cell_values: list, proof) -> bool:
        return self.verifier.multi_verify(commitment, cell_index, cell_values, proof)
...
def reconstruct_extended_blob(domain: RootsOfUnity, cells: dict,
                               D: int = D_CELL_SIZE,
                               k: int = K_CELLS_BLOB,
                               n: int = N_CELLS_EXT) -> list:
    # cells is a dict: {cell_index -> [D field elements]}
    p = domain.p
    m = domain.m
    assert m == n * D, f"domain size {m} must equal n*D = {n*D}"
    assert len(cells) >= k, f"Need at least k={k} cells to reconstruct, got {len(cells)}"
...
class CCfull:
    ...
    def __init__(self, srs: SRS, domain: RootsOfUnity, ell: int,
                 D: int = D_CELL_SIZE, k: int = K_CELLS_BLOB, n: int = N_CELLS_EXT):
    ...
    def verify_column(self, commitments: list, col_index: int,
                      cells: list, proofs: list) -> bool:
    ...
    def batch_verify(self, commitments: list, col_indices: list,
                     row_indices: list, cells: list, proofs: list) -> bool:
